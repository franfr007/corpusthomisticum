...
function renderWorksList() {
        ...
        <div class="filters">
            <div class="filter-group">
                ...
                <select id="categoryFilter" onchange="filterByCategory(this.value)">
                    ${categories.map(cat => `
                        <option value="${cat}" ${cat === state.selectedCategory ? 'selected' : ''}>
                            ${cat === 'all' ? 'Todas' : cat}
                        </option>
                    `).join('')}
                </select>
            ...
            <div class="filter-group">
                <label>Total de obras: <strong>${filteredWorks.length}</strong></label>
            </div>
        </div>
    `;
    ...
    // Renderizar obras agrupadas
    const worksHtml = Object.entries(groupedWorks).map(([category, works]) => `
        <div class="work-category">
            <h3>${category} (${works.length})</h3>
            <div class="works-grid">
                ${works.map(work => `
                    <div class="work-item" onclick="loadWork('${work.id}')">
                        <h4>${work.title}</h4>
                    </div>
                `).join('')}
            </div>
    ...
    worksList.innerHTML = filtersHtml + worksHtml;
}
